...
export default function CrmStatsPage() {
  ...
  if (!stats) {
    return <p className="text-sm text-[#6a6578]">Chargement des statistiques…</p>;
  }
  ...
  return (
    <div className="space-y-8">
      <div className="crm-animate-in">
        ...
        <p className="mt-1 text-sm text-[#8b869c]">Vue d&apos;ensemble de votre prospection.</p>
      </div>

      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 crm-animate-in-delay-1">
        {cards.map((c) => (
          <div key={c.label} className={`${ui.card} p-5`}>
            <p className="text-xs font-medium uppercase tracking-wide text-[#8b869c]">
              {c.label}
            </p>
            <p className="mt-2 text-3xl font-semibold tracking-tight text-[#f3f0fa]">
              {c.value}
            </p>
          </div>
        ))}
        ...
        <section className={`${ui.card} p-5 sm:p-6`}>
          ...
          <div className="mt-5 flex h-40 items-end gap-1">
            {stats.activityByDay.map((d) => (
              <div
                key={d.date}
                className="group relative flex-1 rounded-t-sm bg-violet-500/80 transition hover:bg-violet-400"
                style={{ height: `${Math.max(4, (d.count / maxDay) * 100)}%` }}
                title={`${d.date}: ${d.count}`}
              >
                <span className="pointer-events-none absolute -top-7 left-1/2 hidden -translate-x-1/2 rounded bg-[#14121c] px-1.5 py-0.5 text-[10px] text-[#f3f0fa] group-hover:block">
                  {d.count}
                </span>
              </div>
            ))}
          </div>
          <div className="mt-2 flex justify-between text-[11px] text-[#6a6578]">
