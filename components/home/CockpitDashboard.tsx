  ...
  return (
    <div className="space-y-7">
      <div className="crm-animate-in">
        <p className="text-xs font-medium capitalize tracking-wide text-[#6a6578]">{dateLabel}</p>
        <h1 className={`${ui.h1} mt-1`}>Bonjour {firstName}</h1>
        <p className="mt-1.5 text-sm text-[#8b869c]">Voici ce qui t&apos;attend aujourd&apos;hui.</p>
      </div>
          ...
          return (
            <div key={s.label} className={ui.statCard}>
              <div className="flex items-center justify-between gap-2">
                <p className="text-[10px] font-medium uppercase tracking-[0.08em] text-[#8b869c]">
                  {s.label}
                </p>
                <Icon className="h-3.5 w-3.5 text-violet-400/80" stroke={1.6} />
              </div>
              <p className="mt-2 text-[1.75rem] font-semibold tracking-tight text-[#f3f0fa] tabular-nums">
                {s.value}
              </p>
            </div>
          );
...
function WidgetHeader({
  ...
  return (
    <div className="flex items-center justify-between gap-3 px-4 pt-4 sm:px-5">
      <h2 className="text-sm font-semibold text-[#f3f0fa]">{title}</h2>
      <Link
        href={href}
        className="inline-flex items-center gap-1 text-xs font-medium text-[#8b869c] transition hover:text-violet-300"
      >
        {cta}
          ...
          prospects.map((p) => (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-medium text-[#f3f0fa]">{p.club_name}</p>
                <p className="truncate text-xs text-[#6a6578]">
  ...
  return (
    <section className={ui.widget}>
      ...
      <div className="flex flex-1 flex-col gap-0 p-4 sm:px-5 sm:pb-5">
        {events.length === 0 ? (
          <p className="py-8 text-center text-sm text-[#6a6578]">Aucun événement aujourd&apos;hui.</p>
        ) : (
          <ol className="relative space-y-0 border-l border-white/[0.08] pl-4">
            {events.map((event) => (
              <li key={event.id} className="relative pb-3 last:pb-0">
                ...
                <p className="text-[11px] font-medium tabular-nums text-violet-300">{eventTime(event)}</p>
                <p className="text-sm font-medium text-[#f3f0fa]">{event.title}</p>
                {event.location ? (
                  <p className="text-xs text-[#6a6578]">{event.location}</p>
                ) : null}
              </li>
            ))}
          </ol>
    ...
    <section className={ui.widget}>
      ...
      <div className="flex flex-1 flex-col justify-between gap-5 p-4 sm:p-5">
        <div className="space-y-3">
          <p className="text-2xl font-semibold tracking-tight text-[#f3f0fa]">
            {english.dueToday}{" "}
            <span className="text-sm font-medium text-[#8b869c]">
              carte{english.dueToday > 1 ? "s" : ""} à réviser
            </span>
          </p>
          <p className="flex items-center gap-1.5 text-sm text-[#c8c3d6]">
            <IconFlame className="h-4 w-4 text-orange-400" stroke={1.75} />
            Série : {english.streak} jour{english.streak > 1 ? "s" : ""}
          </p>
          <div>
            <div className="mb-1.5 flex items-center justify-between text-xs text-[#8b869c]">
