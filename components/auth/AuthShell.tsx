      ...
      <main className="relative z-10 flex flex-1 items-center justify-center px-5 pb-12 sm:px-8">
        <div className="crm-animate-in w-full max-w-[400px]">
          <div className="wo-card p-7 sm:p-8">
            <h1 className="font-display text-2xl font-semibold tracking-tight text-[#eef6f2]">{title}</h1>
            {subtitle ? (
              <p className="mt-1.5 text-sm leading-relaxed text-[#8a9e96]">{subtitle}</p>
            ) : null}
            <div className="mt-7">{children}</div>
          </div>
          {footer ? (
            <div className="mt-5 text-center text-sm text-[#8a9e96]">{footer}</div>
          ) : null}
        </div>
      </main>
    </div>
