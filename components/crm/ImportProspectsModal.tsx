...
export function ImportProspectsModal({ open, onClose, onImported }: ImportProspectsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative flex max-h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-[#e8eef6] bg-white shadow-xl">
        {/* Header */}
        <div className="border-b border-[#e8eef6] px-6 py-4">
          <h2 className="text-lg font-semibold text-slate-900">Importer des prospects</h2>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          ...
          {step === "select" ? (
            <div className="space-y-4">
              ...
              <label
                className={`flex cursor-pointer flex-col items-center gap-3 rounded-2xl border-2 border-dashed border-[#e8eef6] bg-slate-50/50 px-6 py-10 transition hover:border-blue-300 hover:bg-blue-50/30 ${loading ? "pointer-events-none opacity-60" : ""}`}
              >
                {loading ? (
                  <IconLoader2 className="h-8 w-8 animate-spin text-blue-600" />
                ) : (
                  <IconUpload className="h-8 w-8 text-slate-400" stroke={1.5} />
                )}
          ...
          {step === "preview" && parsed ? (
            <div className="space-y-6">
              ...
              {/* Mapping colonnes */}
              <section>
                <h3 className="text-sm font-semibold text-slate-900">Association des colonnes</h3>
                ...
                <div className="mt-3 space-y-2">
                  {parsed.columns.map((col) => (
                    <div key={col} className="flex items-center gap-3">
                      <span className="w-2/5 truncate text-sm text-slate-600" title={col}>
                        {col || "(sans nom)"}
                      </span>
              ...
              {/* Doublons */}
              <section>
                <h3 className="text-sm font-semibold text-slate-900">Gestion des doublons</h3>
                <p className="mt-0.5 text-xs text-slate-400">
                    ...
                    <label
                      ...
                      className={`flex cursor-pointer items-start gap-3 rounded-xl border px-4 py-3 transition ${
                        duplicateStrategy === value
                          ...
                          : "border-[#e8eef6] hover:bg-slate-50"
                      }`}
                    >
                      <input
                        type="radio"
              ...
              {/* Aperçu 5 lignes */}
              <section>
                <h3 className="text-sm font-semibold text-slate-900">Aperçu (5 premières lignes)</h3>
                <div className="mt-2 overflow-x-auto rounded-xl border border-[#e8eef6]">
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-[#e8eef6] bg-slate-50">
                        {parsed.columns.map((col, i) => (
                          <th key={i} className="whitespace-nowrap px-3 py-2 text-left font-medium text-slate-500">
                            {col || `Col ${i + 1}`}
                          </th>
                        ))}
                      </tr>
          ...
          {step === "success" && result ? (
            <div className="space-y-4 text-center">
              ...
              <div>
                <h3 className="text-lg font-semibold text-slate-900">Import terminé</h3>
...
function StatCard({
  ...
  return (
    <div className="rounded-xl border border-[#e8eef6] bg-slate-50/50 px-4 py-3">
      <p className="text-xs text-slate-400">{label}</p>
      <p className={`mt-0.5 text-xl font-semibold ${accent ?? "text-slate-900"}`}>{value}</p>
    </div>
  );
}
