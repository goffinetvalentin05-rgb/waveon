...
function bucketize(points: ActivityPoint[], size: number, daily: boolean): Bucket[] {
  ...
  for (let i = 0; i < points.length; i += size) {
    ...
    const emails = slice.reduce((s, p) => s + p.emails, 0);
    const outreach = slice.reduce((s, p) => s + p.calls + p.messages, 0);
    ...
    buckets.push({
      ...
      total: emails + outreach + meetings,
      emails,
      outreach,
      meetings,
    });
...
export function ProspectingCadence({
  ...
  const totals = buckets.reduce(
    (acc, b) => ({
      emails: acc.emails + b.emails,
      outreach: acc.outreach + b.outreach,
      meetings: acc.meetings + b.meetings,
    }),
    { emails: 0, outreach: 0, meetings: 0 }
  );
  ...
  return (
    <section className="wo-widget h-full p-4 lg:p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 flex items-baseline gap-2">
            ...
            <span className="text-[12px] text-wo-muted lg:text-[12.5px]">
              {total === 1 ? "prise de contact" : "prises de contact"} sur {RANGE_LABELS[range].toLowerCase()}
            </span>
          </p>
        </div>
            ...
            <div key={b.key} className="group flex h-full min-w-0 flex-1 flex-col items-center justify-end gap-2">
              <div className="relative flex w-full flex-1 items-end justify-center">
                ...
                <div
                  ...
                  style={{ height: `${height}%` }}
                  title={`${b.total} prise${b.total > 1 ? "s" : ""} de contact`}
                />
              </div>
      ...
      <div className="mt-4 hidden grid-cols-3 gap-2 border-t border-wo-border pt-4 lg:mt-5 lg:grid">
        {[
          { label: "Emails", value: totals.emails, dot: "bg-[#f3a35c]" },
          { label: "Appels & messages", value: totals.outreach, dot: "bg-[#d97732]" },
