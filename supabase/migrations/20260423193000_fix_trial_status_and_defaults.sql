-- ==========================================================
-- Waevon : essai gratuit (source de vérité en base)
-- Objectifs (STRICT) :
-- - À la création : trial_started_at = now(), trial_ends_at = now()+7j
--   subscription_status = 'trialing', subscription_plan = 'trial'
-- - Pendant le trial : accès complet (géré app/middleware)
-- - Après expiration (sans Stripe) : blocage total hors facturation (géré app/middleware)
-- ==========================================================

-- 1) Colonnes trial (idempotent)
...
-- 2) Defaults SQL "safe" (source de vérité en base)
alter table public.wavon_businesses
...
-- Permettre 'trial' côté plan (le check existant n’autorise que starter/pro)
alter table public.wavon_businesses
...
-- 3) Trigger auth.users : initialisation trial (ne touche pas au flux Stripe)
...
-- 4) Rattrapage (safe) : corrige les trials cassés / manquants
